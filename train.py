...
if __name__=="__main__":
    if len(sys.argv) == 3:
        ...
        valid_loader  = torch.utils.data.DataLoader(valid_dataset,batch_size=batch_size,shuffle=False,num_workers=40,pin_memory=True)
            ...
            if 1:
                ...
                if (epoch+1) % save_interval == 0:
                    ...
                    if old_model.network_name == 'yolo_v2':
                        ...
                        extension = 'pth'
                    logging('save weights to %s/%s_v2_%06d.%s' % (backupdir,old_model.network_name,epoch+1,extension))
                    model.module.seen = seen_samples #(epoch + 1) * len(train_loader.dataset)
                    model.module.save_weights('%s/%s_v2_%06d.%s' % (backupdir,old_model.network_name,epoch+1,extension))
